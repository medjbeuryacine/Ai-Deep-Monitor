...
GPU_API_BASE = "http://192.168.1.153:8060/tracking_entree"
...
@router.get("/api/video_entree")
async def video_entree():
    return StreamingResponse(
        proxy_stream("tracking_entree"),
...
@router.get("/api/test_connection_curl", response_class=HTMLResponse)
async def test_connection_curl():
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            response = await client.get("http://192.168.1.153:8060/")
            response.raise_for_status()
