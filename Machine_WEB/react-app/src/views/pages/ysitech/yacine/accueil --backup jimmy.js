...
export default function DataCenterSecurityPage() {
  ...
  const [streamUrl, setStreamUrl] = useState('/api/video_entree');
  ...
  const fetchAvailableFlux = async () => {
    try {
      ...
      if (streams && streams.length > 0) {
        ...
        const fluxList = streams.map(stream => ({
        ...
        // Sélectionner par défaut le premier flux actif
        const defaultFlux = fluxList.find(flux => flux.isActive) || fluxList[0];
  ...
  const fetchAllCameras = async () => {
    try {
      ...
      if (!response.ok) throw new Error('Erreur API caméras');
      ...
      if (cameras && cameras.length > 0) {
        ...
        // Utiliser des valeurs par défaut si aucune caméra n'est disponible
        setAvailableCameras([]);
        setCameraInfo({
          nom_cam: "Caméra entrée test",
      ...
      addAlert('Erreur de chargement des caméras', 'error');
    }
  };

  // Mise à jour de l'URL du flux vidéo en fonction de la caméra ou du flux sélectionné
  const updateStreamUrl = (camera, flux) => {
    let fluxUrl = '/api/video_entree'; // URL par défaut
    
    if (lockMode === 'camera' && camera) {
      // Si on est en mode caméra, on utilise l'URL de la caméra
      if (selectedFlux === 'flux1' && camera.flux_principal_active) {
        fluxUrl = camera.adresse_flux_principal;
      } else if (selectedFlux === 'flux2' && camera.flux_secondaire_active) {
        fluxUrl = camera.adresse_flux_secondaire;
      } else if (selectedFlux === 'flux3' && camera.flux_tertiaire_active) {
        fluxUrl = camera.adresse_flux_tertiaire;
      }
    } else if (lockMode === 'flux' && flux) {
      // Si on est en mode flux, on utilise l'URL du flux
      fluxUrl = flux.url;
    }
    
    // Ajouter un timestamp pour éviter la mise en cache du navigateur
    setStreamUrl(`${fluxUrl || '/api/video_entree'}?t=${Date.now()}`);
  };
...
const handleCameraChange = (event) => {
  ...
  if (selectedCam) {
    ...
    addAlert(`Caméra changée: ${selectedCam.nom_cam}`, 'info');
  }
};
    ...
    <Box sx={{ 
      ...
      <Box sx={{ 
        ...
        <Box sx={{ 
          ...
          width: '100%',
          // Utilisation de dimensions relatives pour s'adapter
          ...
          overflow: 'hidden',
          position: 'relative',
          // Hauteur minimale pour éviter un écrasement excessif
          ...
          <Box sx={{ 
            ...
            bgcolor: 'black',
            overflow: 'hidden',
            // Ajout d'une largeur maximale qui s'adapte à différentes tailles d'écran
            maxWidth: '83.6%',
            // S'assure que la hauteur s'adapte au contenu
            height: 'auto',
            aspectRatio: '16/9', // Maintient un rapport hauteur/largeur standard pour la vidéo
            
          }}>
            {/* Solution simple avec une iframe pour le flux MJPEG */}
            <iframe
              src={streamUrl}
              title={lockMode === 'camera' 
                ? `Flux vidéo: ${cameraInfo.nom_cam}`
                : `Flux vidéo: ${availableFlux.find(f => f.id === selectedFlux)?.name || 'Flux inconnu'}`}
              style={{
                width: '100%',
                height: '100%',
                border: 'none',
                backgroundColor: 'black',
                objectFit: 'contain', // S'assure que la vidéo s'adapte sans être déformée
                maxWidth: '100%',
                maxHeight: '100%',
              }}
              
              onError={() => {
                addAlert(`Erreur de chargement du flux`, 'error');
              }}
            />
