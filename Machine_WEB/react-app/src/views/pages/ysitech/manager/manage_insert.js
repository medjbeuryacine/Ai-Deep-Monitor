...
const DeviceManagement = () => {
  ...
  // Fonction pour gérer le tri des dispositifs
  ...
  // Fonction pour gérer le tri de l'historique
  ...
  // Fonction pour appliquer les filtres sur les dispositifs
  const filteredDeviceList = deviceList.filter((device) => {
    return (
      ...
      device.ip.toLowerCase().includes(filters.ip.toLowerCase()) &&
  ...
  // Fonction pour appliquer les filtres sur l'historique
  const filteredHistoryList = historyList.filter((entry) => {
    return (
      entry.serial.toLowerCase().includes(historyFilters.serial.toLowerCase()) &&
      entry.action.toLowerCase().includes(historyFilters.action.toLowerCase()) &&
      entry.type.toLowerCase().includes(historyFilters.type.toLowerCase()) &&
      entry.old_value.toLowerCase().includes(historyFilters.old_value.toLowerCase()) &&
      entry.new_value.toLowerCase().includes(historyFilters.new_value.toLowerCase()) &&
      entry.created_at.toLowerCase().includes(historyFilters.created_at.toLowerCase())
    );
  });
  ...
  const sortedDeviceList = sortConfig.key
    ? sortData(filteredDeviceList, sortConfig.key, sortConfig.direction)
    : filteredDeviceList;

  const sortedHistoryList = historySortConfig.key
    ? sortData(filteredHistoryList, historySortConfig.key, historySortConfig.direction)
    : filteredHistoryList;
  ...
  return (
    <Box sx={{ padding: 2 }}>
      ...
      {/* Filtres pour les dispositifs */}
      <Grid container spacing={2} sx={{ marginTop: 2 }}>
        {['serial', 'name', 'ip', 'location', 'status'].map((column) => (
          <Grid item xs={2} key={column}>
            <TextField
              label={`Filtrer par ${column.charAt(0).toUpperCase() + column.slice(1)}`}
              variant="outlined"
              fullWidth
              name={column}
          ...
          <Paper sx={{ padding: 2 }}>
            <TableContainer>
              <Table>
                <TableHead>
                  <TableRow>
                    {['serial', 'name', 'ip', 'status', 'location'].map((column) => (
                      <TableCell
                        key={column}
        ...
        <Grid item xs={12}>
          <Paper sx={{ padding: 2 }}>
            ...
            <Grid container spacing={2} sx={{ marginBottom: 2 }}>
              {['serial', 'action', 'type', 'old_value', 'new_value', 'created_at'].map((column) => (
                <Grid item xs={2} key={column}>
                  <TextField
                    label={`Filtrer par ${column.charAt(0).toUpperCase() + column.slice(1)}`}
                    variant="outlined"
                    fullWidth
                    name={column}
                    value={historyFilters[column]}
                    onChange={(e) => setHistoryFilters({ ...historyFilters, [column]: e.target.value })}
                  />
                </Grid>
              ))}
            </Grid>
              ...
              <Table>
                <TableHead>
                  <TableRow>
                    {['serial', 'action', 'type', 'old_value', 'new_value', 'created_at'].map((column) => (
                      <TableCell
                        key={column}
                        onClick={() => handleHistorySort(column)}
                        style={{ cursor: 'pointer' }}
                      >
                        <span>{column.charAt(0).toUpperCase() + column.slice(1)}</span>
                        {historySortConfig.key === column && (
                          <span>{historySortConfig.direction === 'asc' ? ' ↑' : ' ↓'}</span>
                        )}
                      </TableCell>
                    ))}
                  </TableRow>
                ...
                <TableBody>
                  {sortedHistoryList.length > 0 ? (
                    sortedHistoryList.map((entry, index) => (
                      <TableRow key={index}>
      ...
      <Dialog open={openModal} onClose={() => setOpenModal(false)}>
        ...
        <DialogActions>
          ...
          <Button onClick={handleAddOrUpdateDevice} color="primary">
